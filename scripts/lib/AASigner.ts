...
import {ClutchWallet, ClutchWallet__factory, EntryPoint, EntryPoint__factory} from "../../typechain";
...
export class AASigner extends Signer {
  ...
  async connectWalletAddress(address: string) {
    ...
    this._wallet = ClutchWallet__factory.connect(address, this.signer)
  ...
  async _deploymentTransaction(): Promise<BytesLike> {
    ...
    // @ts-ignore
    return new ClutchWallet__factory()
  ...
  async getWallet(): Promise<ClutchWallet> {
